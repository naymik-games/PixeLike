...
class Room {
   constructor(center, start, end, tl, tr, bl, br) {
...
Room.prototype.roomBetween = function (room) {
   let testRooms = dungeon.rooms.filter(x => x.id != this.id && x.id != room.id);
...
Room.prototype.connectRoom = function (room, min = 3) {
   ...
   if (!success) {
      ...
      if ((vert || dungeon.inRoom(horizCorner)) && !dungeon.inRoom(vertCorner)) {
         success = this.cornerVert(room, vertCorner);
      }
      if (!success && !dungeon.inRoom(horizCorner)) {
...
Room.prototype.cornerVert = function (room, corner) {
   ...
   */
   ...
   if (!vert.isAdjacentVert() &&
      ...
      dungeon.addPath(vert);
      dungeon.addPath(horiz);
...
Room.prototype.addVertPath = function (room, path, wall) {
   ...
   if (path.allowed) {
      ...
      dungeon.addPath(path);

      //dungeon.placeDoor(room)
      this.addNeighbor(room);
   }
   return path;
...
Room.prototype.findPotentialRooms = function () {
   let rooms = dungeon.rooms.filter(x => x.id != this.id);
