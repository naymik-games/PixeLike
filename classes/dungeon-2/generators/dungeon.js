...
class Dungeon extends Generator {
    ...
    getTileMap() {
        ...
        //creat corridor object
        for (let i = 0; i < this.children.length; i++) {
            const piece = this.children[i];
            // console.log(piece.tag)
            if (piece.tag == '') {
                var cen = piece.get_center_pos()
                this.corridorData.push({
                    id: piece.id,
                    tag: 'corridor',
                    center: {
                        x: cen[0],
        ...
        var mapData = []
        for (var y = 0; y < walls.rows.length; y++) {
            var temp = []
            for (var x = 0; x < walls.rows[0].length; x++) {
                if (walls.rows[y][x]) {
                    temp.push(0)
                } else {
                    temp.push(2)
        ...
        for (let i = 0; i < this.children.length; i++) {
            const piece = this.children[i];
            for (var y = 0; y < piece.walls.rows.length; y++) {

                for (var x = 0; x < piece.walls.rows[0].length; x++) {
